...
public class SearchService {
    ...
    private SearchResponseVo parseSearchResult(SearchResponse response){
        ...
        if (!CollectionUtils.isEmpty(brandBuckets)){
            responseVo.setBrands(brandBuckets.stream().map(brandBucket -> {
                BrandEntity brandEntity = new BrandEntity();
                //获取attrAgg中的key, 这个key就是品牌的id
                brandEntity.setId(brandBucket.getKeyAsNumber().longValue());
                //品牌子聚合的map结构
                Map<String, Aggregation> subAggregationMap = brandBucket.getAggregations().asMap();
        ...
        if (!CollectionUtils.isEmpty(categoryBuckets)){
            responseVo.setCategories(categoryBuckets.stream().map(categoryBucket -> {
                ...
                List<? extends Terms.Bucket> categoryNameBucket = categoryNameAgg.getBuckets();
                if (!CollectionUtils.isEmpty(categoryNameBucket)){
                    categoryEntity.setName(categoryNameBucket.get(0).getKeyAsString());
                }
        ...
        if (!CollectionUtils.isEmpty(attrIdAggBuckets)){
            List<SearchResponseAttrVo> filters = attrIdAggBuckets.stream().map(bucket -> {
                ...
                if (!CollectionUtils.isEmpty(buckets)){
                    responseAttrVo.setAttrValues(buckets.stream()
   ...
   private SearchSourceBuilder buildDsl(SearchParamVo paramVo){
        ...
        // 1.2.3. 构建价格区间过滤
        Double priceForm = paramVo.getPriceFrom();
